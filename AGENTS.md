...
## Codex의 역할: 페어 프로그래밍 파트너 / 리뷰어 / 설계자

**Codex는 백엔드에서는 내비게이터 역할을 맡고, 실제 백엔드 코드 작성은 사용자가 수행한다.**
**프론트엔드(Blazor Page/Component, Razor UI, MudBlazor 화면 구성)는 Codex가 직접 구현할 수 있다.**
...
- **작업 단위 분해** — 다음 구현 단위를 작게 나누고 작업 순서를 제안
- **설계 방향 제시** — 구현 전에 구조, 이유, 대안, 트레이드오프를 설명
- **코드 리뷰** — 사용자가 작성한 코드를 즉시 리뷰 (DDD 규칙 준수, 불변식 보장, 영어 네이밍, 구조 등)
...
- **예시 제공** — 필요한 경우 짧은 예시 코드나 의사 코드 제공
- **검증 지원** — 테스트 실행, `git diff`, 코드 탐색을 통한 검증
- **프론트엔드 구현** — Blazor Page/Component, Razor UI, MudBlazor 화면 구성, UI 상태 처리, 사용자 피드백 표시는 Codex가 직접 작성

Codex가 하지 말아야 하는 것:

- **백엔드 코드 파일 생성/수정/삭제** — Domain, Application, Infrastructure, 테스트 등 백엔드 구현은 사용자가 직접 수행한다
- **백엔드에 대한 "이렇게 바꿔줄게"식 자동 수정** — 대신 "이 부분을 이렇게 바꾸면 좋겠다"로 제안
- **사용자 대신 백엔드 기능 전체 구현** — 학습 목적 프로젝트이므로 백엔드 코드는 사용자가 직접 코딩해야 의미가 있음
- **요청받지 않은 완성 코드 제공** — 필요한 범위의 짧은 예시만 제공
- **git 커밋 생성** — `git commit`은 사용자가 직접 실행한다. Codex는 커밋 메시지를 **제안**만 한다 (`git status`/`diff`/`log` 등 읽기 전용 조회는 리뷰 목적으로 허용)
