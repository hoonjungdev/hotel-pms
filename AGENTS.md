...
use-case vertical slice는 기본 배치 규칙이지만 절대 규칙은 아니다. 처음에는 특정 use-case 안에 있던 개념이라도, 둘 이상의 use-case가 공유할 도메인 개념이 되거나 미래 use-case의 자연스러운 입력/출력으로 드러나면 feature 루트의 `Domain` 또는 `Infrastructure` 모듈로 승격한다. 예를 들어 `Reservation Availability`처럼 예약 생성과 가용성 조회가 함께 사용할 개념은 `CreateReservation` 안의 private helper로 숨기지 않는다. 순수 계산/결과 모델은 `Domain`에 두고, EF Core query처럼 persistence가 필요한 구현은 `Infrastructure`에 둔다.
...
- 특정 use-case에서 시작한 도메인 개념이라도 둘 이상의 use-case가 공유하게 되면 feature 루트로 승격한다. 단, 승격은 실제 중복이나 명확한 cross-use-case 필요가 생겼을 때만 한다.
